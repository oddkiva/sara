...
- Another way to calculate the plane normal is to calculate the line
  ...
  \begin{equation}
  \mathbf{\Pi}(\mathbf{a}, \mathbf{b}) =
    \mathbf{K}^\top \mathbf{l}.
  \end{equation}

  $\mathbf{K}^\top \mathbf{l}$ is the plane backprojected from the 2D image line $\mathbf{l}$.

  ```{proof}
  $\mathbf{K}^\top \mathbf{l}$ is indeed the normal of the backprojected plane
  ...
  \begin{equation}
    (\mathbf{K}^\top \mathbf{l})^\top \mathbf{K}^{-1} \mathbf{a} =
    \mathbf{l}^\top \mathbf{K} \mathbf{K}^{-1} \mathbf{a} = 0
  \end{equation}
  ...
  backprojected rays is therefore orthogonal to $\mathbf{K}^\top \mathbf{n}$.
  ```
