...
namespace DO { namespace Sara {
                                               ...
                                               RegionGrowingAnalyzer* analyzer)
  {
    ...
    SARA_DEBUG << "Initalizing R and dR for seed " << _seed << std::endl;
...
//#pragma omp parallel for
        for (int mi = 0; mi < vec_dR.size(); ++mi)
          q[mi].resize(4);
...
//#pragma omp parallel for
        for (int mi = 0; mi < vec_dR.size(); ++mi)
        {
          ...
          if (!find_triple(&q[mi][0], N_K_cap_R[mi], drawer))
            continue;

          if (affine_consistent(&q[mi][0], spurious[mi], drawer))
          {
            isGoodMatch[mi] = 1;
//#pragma omp critical
            {
              analyze_quadruple(&q[mi][0], analyzer);
            }
          }
        }
#ifdef PROFILE_REGION_GROWING
                                       ...
                                       const RegionBoundary& dR) const
    {
      ...
      if (_verbose)
        SARA_DEBUG << "Choose seed triple heuristically" << std::endl;
      ...
      for (++tt; tt != dR.end(); ++tt)
      {
        ...
        {
          if (_verbose)
            SARA_DEBUG << "Found seed triple" << std::endl;
          return true;
        }
      }

      if (_verbose)
        SARA_DEBUG << "Failed to build seed triple" << std::endl;
...
#define I_KNOW_THE_ACCURACY_OF_LOCAL_AFFINITY
    ...
    void GrowRegion::check_local_affine_consistency(
        ...
        const Match m[4], const PairWiseDrawer* drawer) const
    {
      if (drawer)
      {
        // Check visually.
