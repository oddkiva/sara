...
namespace DO { namespace Shakti {
  ...
  std::string Device::formatted_string() const
  {
    ...
    os << format("  CUDA Capability Major/Minor version number:    %d.%d\n",
                 properties.major, properties.minor);

    os << format("  Total amount of global memory:                 %.0f MBytes "
                 "(%llu bytes)\n",
    ...
    os << format("  Maximum Texture Dimension Size (x,y,z)         1D=(%d), "
                 "2D=(%d, %d), 3D=(%d, %d, %d)\n",
                 properties.maxTexture1D, properties.maxTexture2D[0],
                 properties.maxTexture2D[1], properties.maxTexture3D[0],
                 properties.maxTexture3D[1], properties.maxTexture3D[2]);

    os << format(
        "  Maximum Layered 1D Texture Size, (num) layers  1D=(%d), %d layers\n",
        properties.maxTexture1DLayered[0], properties.maxTexture1DLayered[1]);

    os << format("  Maximum Layered 2D Texture Size, (num) layers  2D=(%d, "
                 "%d), %d layers\n",
                 properties.maxTexture2DLayered[0],
                 properties.maxTexture2DLayered[1],
                 properties.maxTexture2DLayered[2]);

    os << format("  Total amount of constant memory:               %lu bytes\n",
                 properties.totalConstMem);

    os << format("  Total amount of shared memory per block:       %lu bytes\n",
                 properties.sharedMemPerBlock);

    os << format("  Total number of registers available per block: %d\n",
                 properties.regsPerBlock);

    os << format("  Warp size:                                     %d\n",
                 properties.warpSize);

    os << format("  Maximum number of threads per multiprocessor:  %d\n",
                 properties.maxThreadsPerMultiProcessor);

    os << format("  Maximum number of threads per block:           %d\n",
    ...
    os << format("  Maximum memory pitch:                          %lu bytes\n",
                 properties.memPitch);

    os << format("  Texture alignment:                             %lu bytes\n",
                 properties.textureAlignment);

    os << format("  Concurrent copy and kernel execution:          %s with %d "
                 "copy engine(s)\n",
                 (properties.deviceOverlap ? "Yes" : "No"),
                 properties.asyncEngineCount);

    os << format("  Run time limit on kernels:                     %s\n",
                 properties.kernelExecTimeoutEnabled ? "Yes" : "No");

    os << format("  Integrated GPU sharing Host Memory:            %s\n",
                 properties.integrated ? "Yes" : "No");

    os << format("  Support host page-locked memory mapping:       %s\n",
                 properties.canMapHostMemory ? "Yes" : "No");

    os << format("  Alignment requirement for Surfaces:            %s\n",
                 properties.surfaceAlignment ? "Yes" : "No");

    os << format("  Device has ECC support:                        %s\n",
                 properties.ECCEnabled ? "Enabled" : "Disabled");

#if defined(WIN32) || defined(_WIN32) || defined(WIN64) || defined(_WIN64)
    os << format("  CUDA Device Driver Mode (TCC or WDDM):         %s\n",
                 properties.tccDriver ? "TCC (Tesla Compute Cluster Driver)"
                                      : "WDDM (Windows Display Driver Model)");
#endif

    os << format("  Device supports Unified Addressing (UVA):      %s\n",
                 properties.unifiedAddressing ? "Yes" : "No");

    os << format("  Device PCI Bus ID / PCI location ID:           %d / %d\n",
  ...
  int get_num_cuda_devices()
  {
    ...
    if (error_id != cudaSuccess)
    {
      std::string error_msg =
          format("cudaGetDeviceCount returned %d\n-> %s\n",
  ...
  std::vector<Device> get_devices()
  {
    ...
    for (auto id = 0; id < num_devices; ++id)
    {
