...
cumulate the locations of all the pixels in the MSER as follows:

\begin{equation}
  \bSigma_x^{-1} = \begin{bmatrix}
    \sum_{\mathbf{x}'} (\mathbf{x}'_1 - \mathbf{x}_1)^2 &
    %
    \sum_{\mathbf{x}'}
      (\mathbf{x}'_1 * \mathbf{x}'_2 -
      \bar{\mathbf{x}}_1 \bar{\mathbf{x}}_2)^2 \\
    %
    \sum_{\mathbf{x}'}
      (\mathbf{x}'_1 * \mathbf{x}'_2 -
    \bar{\mathbf{x}}_1 \bar{\mathbf{x}}_2)^2 &
    %
    \sum_{\mathbf{x}'} (\mathbf{x}'_2 - \mathbf{x}_2)^2
  \end{bmatrix},
\end{equation}
