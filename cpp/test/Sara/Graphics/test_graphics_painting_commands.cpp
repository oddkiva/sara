...
BOOST_AUTO_TEST_CASE(test_draw_point)
{
  ...
  BOOST_CHECK(draw_point(10, 10, Rgba8(255, 0, 0, 0)));
...
BOOST_AUTO_TEST_CASE(test_display_rgb8_image)
{
  const auto w = 50, h = 50;
  const auto data = std::vector<Rgb8>(w * h, Red8);
  BOOST_CHECK(display(ImageView<const Rgb8>{data.data(), {w, h}}, {10, 10}, 2.));
}

BOOST_AUTO_TEST_CASE(test_display_rgba8_image)
{
  const auto w = 50, h = 50;
  const auto data = std::vector<Rgba8>(w * h, Rgba8{0, 0, 0, 255});
  BOOST_CHECK(display(ImageView<const Rgba8>{data.data(), {w, h}}, {10, 10}, 2.));
}

BOOST_AUTO_TEST_CASE(test_display_grayscale_image)
{
  const auto w = 50, h = 50;
  const auto data = std::vector<unsigned char>(w * h, 0);
  BOOST_CHECK(display(ImageView<const std::uint8_t>{data.data(), {w, h}}, {10, 10}, 2.));
}

BOOST_AUTO_TEST_CASE(test_display)
{
  auto image = Image<Rgb8>{50, 50};
  image.flat_array().fill(Red8);
  BOOST_CHECK(display(image, Point2i::Zero(), 1.4));
  BOOST_CHECK(display(image, Point2i(0, 10), 1.4));

  auto rgb_image = Image<Rgb8>{50, 50};
  rgb_image.flat_array().fill(Red8);
  BOOST_CHECK(display(rgb_image, Point2i::Zero(), 1.4));
