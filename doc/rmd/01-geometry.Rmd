...
[@Ok:2013:phdthesis]. Here I am retouching my writing again to make it less dry.
The modest contribution of this text is to break down a bit more the
...
segment and the elliptical arc $(\theta_1, \theta_2)$ such that $|\theta_2 -
\theta_1| \leq \pi$.
...
To summarize, our portion of interest, highlighted by the elliptic portion in
Figure \@ref(fig:ellsector2), has an area which is equal to
...
2}$. In this case, the two equations \@ref(eq:twoellipses) simultaneously
...
$x$. Necessarily the null space $\mathrm{Null}(\mathbf{B}(y))$ is always
...
consider two ellipses $(\mathcal{E}_i)_{1 \leq i \leq 2}$ and we
respectively denote
...
```{r, echo = FALSE}
knitr::include_graphics("figures/test0a.png")
knitr::include_graphics("figures/test0b.png")
knitr::include_graphics("figures/test1a.png")
knitr::include_graphics("figures/test1b.png")
```
...
Figure \@ref(figinter2), which [@Eberly] apparently forgot to consider.  Namely
the cases correspond to whether the center of ellipses $\mathcal{E}_1$ and
$\mathcal{E}_2$ are on the same side or on opposite side with respect to the
line $(\mathbf{p}_1, \mathbf{p}_2)$.
...
$(\mathbf{p}_1, \mathbf{p}_2)$ by $\mathbf{n}$ (cf.  Figure `1.9
<#fig:inter2>`__). If the ellipse centers $\mathbf{x}_1$ and $\mathbf{x}_2$ are
on opposite side with respect to the line $(\mathbf{p}_1, \mathbf{p}_2)$,
*i.e.*,
...
If they are on the same side with respect to the line $(\mathbf{p}_1,
\mathbf{p}_2)$, *i.e.*,
...
Note that the condition $|\langle\mathbf{n},\mathbf{x}_1-\mathbf{p}_1\rangle|
\leq |\langle\mathbf{n},\mathbf{x}_2-\mathbf{p}_1\rangle|$ in
Equation \@ref(eq:inter2b) just expresses the fact that the distance of ellipse
center $\mathbf{x}_1$ to the line $(\mathbf{p}_1, \mathbf{p}_2)$ is smaller than
the distance of ellipse center $\mathbf{x}_2$ to the line $(\mathbf{p}_1,
\mathbf{p}_2)$.
