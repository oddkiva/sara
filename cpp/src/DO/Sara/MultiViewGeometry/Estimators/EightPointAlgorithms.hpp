...
namespace DO { namespace Sara {
  ...
  inline void eight_point_fundamental_matrix(const Matrix<double, 3, 8>& x,
                                             ...
                                             Matrix3d& F)
  {
    // 1. solve the linear system from the 8-point correspondences.
    {
      ...
      Eigen::JacobiSVD<Matrix<double, 8, 9>> svd(A, Eigen::ComputeFullV);
      const Matrix<double, 9, 1> S = svd.singularValues();
    ...
    {
      auto svd = Eigen::JacobiSVD<Matrix3d>{
          F, Eigen::ComputeFullU | Eigen::ComputeFullV};
      Vector3d D = svd.singularValues();
      D(2) = 0;
      F = svd.matrixU() * D.diagonal() * svd.matrixV().transpose();
    }
  }

  inline void four_point_homography(const Matrix<double, 3, 4>& x,
