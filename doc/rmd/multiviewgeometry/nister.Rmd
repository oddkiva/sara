...
This method is originally published in CVPR [@nister:2003:cvpr] and republished
at length in PAMI [@nister:2004:pami]. It solves the essential matrix from $5$
point correspondences. Here I revisit the PAMI article [@nister:2004:pami] which
I found clearer.
...
- from this set of $10$ polynomial equations that are simultaneously satisfied
  amounts to solving only a set of $3$ easier polynomial equations;
...
following groups of monomials:

- $\left\{ x, x z, x z^2, x z^3 \right\}$
- $\left\{ y, y z, y z^2, x z^3 \right\}$
- $\left\{ 1, z, z^2, z^3, z^4 \right\}$

In turn, the very nice thing is that when we group coefficients using these
three subgroups, we see respectively that:
...
This is interesting because the system of polynomial equations \@ref(eq:klm) is
rewritten as
