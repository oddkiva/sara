...
NvEncoderOutputInVidMemD3D11::NvEncoderOutputInVidMemD3D11(
    ID3D11Device* pD3D11Device, uint32_t nWidth, uint32_t nHeight,
    NV_ENC_BUFFER_FORMAT eBufferFormat, bool bMotionEstimationOnly)
...
uint32_t NvEncoderOutputInVidMemD3D11::GetOutputBufferSize()
{
  ...
  if (m_bMotionEstimationOnly)
  {
    ...
    bufferSize =
        encodeWidthInMbs * encodeHeightInMbs * sizeof(NV_ENC_H264_MV_DATA);
  }
  else
  {
...
void NvEncoderOutputInVidMemD3D11::AllocateOutputBuffers(
    uint32_t numOutputBuffers)
{
  ...
  for (uint32_t i = 0; i < numOutputBuffers; i++)
  {
    ...
    if (m_pD3D11Device->CreateBuffer(&desc, NULL, (ID3D11Buffer**) &dx11bfr) !=
        S_OK)
    {
      NVENC_THROW_ERROR("Failed to create ID3D11Buffer",
...
{
  NV_ENC_BUFFER_USAGE bufferUsage = m_bMotionEstimationOnly
  ...
  for (uint32_t i = 0; i < m_pOutputBuffers.size(); ++i)
  {
    if (m_pOutputBuffers[i])
    {
      NV_ENC_REGISTERED_PTR registeredPtr = RegisterResource(
          m_pOutputBuffers[i], NV_ENC_INPUT_RESOURCE_TYPE_DIRECTX, bfrSize, 1,
          0, NV_ENC_BUFFER_FORMAT_U8, bufferUsage);
  ...
  for (uint32_t i = 0; i < m_vRegisteredResourcesOutputBuffer.size(); ++i)
  {
    if (m_vRegisteredResourcesOutputBuffer[i])
    {
      m_nvenc.nvEncUnregisterResource(m_hEncoder,
                                      m_vRegisteredResourcesOutputBuffer[i]);
    }
  }
  m_vRegisteredResourcesOutputBuffer.clear();
}

void NvEncoderOutputInVidMemD3D11::CreateEncoder(
...
void NvEncoderOutputInVidMemD3D11::MapResources(uint32_t bfrIdx)
{
  ...
  // map output surface
  NV_ENC_MAP_INPUT_RESOURCE mapInputResourceBitstreamBuffer = {
      NV_ENC_MAP_INPUT_RESOURCE_VER};
  mapInputResourceBitstreamBuffer.registeredResource =
      m_vRegisteredResourcesOutputBuffer[bfrIdx];
  NVENC_API_CALL(m_nvenc.nvEncMapInputResource(
      m_hEncoder, &mapInputResourceBitstreamBuffer));
  m_vMappedOutputBuffers[bfrIdx] =
      mapInputResourceBitstreamBuffer.mappedResource;
}

void NvEncoderOutputInVidMemD3D11::EncodeFrame(
    ...
    NV_ENC_PIC_PARAMS* pPicParams)
{
  ...
  NVENCSTATUS nvStatus = DoEncode(m_vMappedInputBuffers[bfrIdx],
                                  m_vMappedOutputBuffers[bfrIdx], pPicParams);
...
void NvEncoderOutputInVidMemD3D11::EndEncode(
    std::vector<NV_ENC_OUTPUT_PTR>& pOutputBuffer)
{
  if (!IsHWEncoderInitialized())
  {
...
void NvEncoderOutputInVidMemD3D11::RunMotionEstimation(
    std::vector<NV_ENC_OUTPUT_PTR>& pOutputBuffer)
{
  pOutputBuffer.clear();
...
void NvEncoderOutputInVidMemD3D11::GetEncodedPacket(
    std::vector<NV_ENC_OUTPUT_PTR>& pOutputBuffer, bool bOutputDelay)
{
  unsigned i = 0;
