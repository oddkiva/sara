...
namespace v3 {
  ...
  public:
    ...
    Input<Buffer<float>[2]> polar_gradient { "gradients", 4 };
    Input<Buffer<float>[3]> xys { "xys", 1 };
    Input<Buffer<std::int32_t>> scale_index { "scale_index", 1 };
    Input<Buffer<std::int32_t>> batch_index { "batch_index", 1 };
    ...
    void generate()
    {
      ...
      const auto x = xys[0](k);
      const auto y = xys[1](k);
      const auto scale = xys[2](k);
      const auto s_index = scale_index(k);
      const auto n_index = batch_index(k);
