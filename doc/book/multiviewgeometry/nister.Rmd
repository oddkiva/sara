...
\begin{equation}
  \mathbf{Q}_i =  \mathrm{vec}(\mathbf{p}_{i} \mathbf{q}_i^\top)
\end{equation}
...
1. the essential matrix is also characterized equivalently by the matrix
   ...
   \begin{equation}
     ...
     \end{bmatrix}
     \iff
     \bE \bE^\top \bE - \frac{1}{2} \mathrm{trace}(\bE\bE^\top) \bE = 0
   \end{equation}
...
From this matrix notation, it appears that $[x, y, 1]^\top$ is a vector in the
