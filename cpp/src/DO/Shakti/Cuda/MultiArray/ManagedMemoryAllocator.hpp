...
namespace DO { namespace Shakti {
  ...
  template <>
  class ManagedMemoryAllocator<void>
  {
  public:
    ...
    struct rebind
    {
      using other = ManagedMemoryAllocator<U>;
    };
  };
  ...
  public:
    ...
    __host__ __device__ inline ManagedMemoryAllocator()
    {
    }

    template <typename U>
    __host__ __device__ inline ManagedMemoryAllocator(
        const ManagedMemoryAllocator<U>&)
    {
    }
    ...
    __host__ inline pointer allocate(size_type cnt, const_pointer = 0)
    {
      ...
      auto error = cudaMallocManaged(reinterpret_cast<void**>(&result),
    ...
    __host__ __device__ inline bool operator==(ManagedMemoryAllocator const&)
    {
      return true;
    }

    __host__ __device__ inline bool operator!=(ManagedMemoryAllocator const& x)
    {
      return !operator==(x);
    }
  };
