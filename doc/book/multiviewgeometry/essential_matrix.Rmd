...
vector normal to the plane induced by the $\mathrm{span}(\mathbf{t}$,
...
\begin{equation}
  ...
  %
  \mathbf{X}^0 + \mathbf{R^\top} \mathbf{t} &= \mathbf{R}^\top \mathbf{X}^1 \\
  %
  \mathbf{X}^0 &= \mathbf{R}^\top \mathbf{X}^1 - \mathbf{R^\top} \mathbf{t} \\
  %
  \end{aligned}
\end{equation}
...
- the position of camera $C_1$ w.r.t. camera coordinate system $C_0$,
  is calculated as $\mathbf{t}_{0 \rightarrow 1} = -\mathbf{R}^\top \mathbf{t}$.
...
- The coordinates of the second camera center in the first camera coordinate
  system is $-\mathbf{R}^\top \mathbf{t}$.
...
## Some (False) Understanding about Incremental Bundle Adjustment
...
\begin{equation}
  \mathbf{R}_{j} = \mathbf{R}_{i} \mathbf{R}_{ij} \mathbf{R}_{i}^\top \\
  %
