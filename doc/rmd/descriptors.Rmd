...
At this time, this chapter contains only one section where I write about the
SIFT descriptor.
...
:::fyi
In this section, I clarify one big confusion that I had when comparing the
...
In fact the drawing that illustrates the local image descriptor in
...
from which we calculate their histogram of gradients.  Each histogram bins gradient
...
In other words, this is so that we can compensate for the *noisy* estimation of
keypoint $k$. If we did not apply the trilinear interpolation, that actually
means that the grid of patches are mutually disjoint as illustrated in Figure
\@ref(fig:patches-nonoverlapping). Because of the noisy estimation, a pixel at the
boundary of a subpatch patch could then fall suddenly in another adjacent image
patch. In turn this would have a catastrophic effect in the performance of the
SIFT descriptor if we cumulate the contributions of all such pixels.
...
interpolation. That mean that we have to increase the radius of each subpatch
...
trilinear interpolation, we compensate for the noisy orientation $\theta$. This
