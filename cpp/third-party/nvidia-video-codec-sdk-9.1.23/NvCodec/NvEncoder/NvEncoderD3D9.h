...
{
public:
  NvEncoderD3D9(IDirect3DDevice9* pD3D9Device, uint32_t nWidth,
                uint32_t nHeight, NV_ENC_BUFFER_FORMAT eBufferFormat,
                IDirectXVideoAccelerationService* pDXVAService = nullptr,
...
private:
   ...
   * NvEncoder::AllocateInputBuffers(). This function creates IDirect3DSurface9*
   * which is used to accept input data. To obtain handle to input buffers
   * application must call NvEncoder::GetNextInputFrame()
   */
...
private:
  /**
   *  @brief This is a private function to release IDirect3DSurface9 surfaces
   * used for encoding.
   */
