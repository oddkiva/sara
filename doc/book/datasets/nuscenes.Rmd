...
Essentially, from a careful examination, the groups of concepts emerge as
follows:

- a *sample* and its associated collection of *sampled data* (images, point
  clouds, velocity fields)
- an object *annotation*:
  - grouped into *instances* for object tracking purposes
  - categorized according to a taxonomy of road users.
  - characterized by a set of *attributes* and *visibility*.
- an *ego-pose* that quantifies the car global positioning
- a *calibration* data used to relate the 3D geometry with respect to each
  sensor.
...
Let us already state that each 3D box has a point of reference expressed in the
global coordinate system. And annotations are grouped into tracks which nuScenes
terms as *Instances* which is used for the purpose of object tracking
evaluation.
...
2. the radar poses with respect to the IMU coordinate system
   \begin{equation}
     \begin{bmatrix}
       \mathbf{R}_{r / \text{IMU}}^\top & \mathbf{0}_3 \\
       %
       \mathbf{0}_3^\top & 1
     \end{bmatrix}
     %
...
3. the lidar pose with respect to the IMU coordinate system
   \begin{equation}
     \begin{bmatrix}
       \mathbf{R}_{l / \text{IMU}}^\top & \mathbf{0}_3 \\
       %
       \mathbf{0}_3^\top & 1
     \end{bmatrix}
     %
