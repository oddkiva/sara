...
NvEncoderD3D9::NvEncoderD3D9(IDirect3DDevice9* pD3D9Device, uint32_t nWidth,
                             uint32_t nHeight,
                             NV_ENC_BUFFER_FORMAT eBufferFormat,
                             IDirectXVideoAccelerationService* pDXVAService,
                             uint32_t nExtraOutputDelay,
                             bool bMotionEstimationOnly)
  : NvEncoder(NV_ENC_DEVICE_TYPE_DIRECTX, pD3D9Device, nWidth, nHeight,
...
void NvEncoderD3D9::AllocateInputBuffers(int32_t numInputBuffers)
{
  ...
  for (int count = 0; count < numCount; count++)
  {
    ...
    for (int i = 0; i < numInputBuffers; i++)
    {
      ...
      if (m_pDXVAService)
      {
        res = m_pDXVAService->CreateSurface(
            GetMaxEncodeWidth(), GetMaxEncodeHeight(), 0,
            GetD3D9Format(GetPixelFormat()), D3DPOOL_DEFAULT, 0,
            DXVA2_VideoProcessorRenderTarget, &pD3D9Surface, nullptr);
      }
      else
      {
        res = m_pD3D9Device->CreateOffscreenPlainSurface(
            GetMaxEncodeWidth(), GetMaxEncodeHeight(),
            GetD3D9Format(GetPixelFormat()), D3DPOOL_DEFAULT, &pD3D9Surface,
            nullptr);
      }
      if (res != S_OK)
      {
        NVENC_THROW_ERROR("Failed to create d3d9Surfaces",
...
void NvEncoderD3D9::ReleaseD3D9Resources()
{
  ...
  for (uint32_t i = 0; i < m_vInputFrames.size(); ++i)
  {
    ...
    {
      reinterpret_cast<IDirect3DSurface9*>(m_vInputFrames[i].inputPtr)
          ->Release();
    }
  }
  ...
  for (uint32_t i = 0; i < m_vReferenceFrames.size(); ++i)
  {
    ...
    {
      reinterpret_cast<IDirect3DSurface9*>(m_vReferenceFrames[i].inputPtr)
          ->Release();
    }
  }
