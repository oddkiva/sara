...
- Another way to calculate the plane normals is to use the normal vector
  $\mathbf{n} = \mathbf{a} \times \mathbf{b}$ of the image line segment
  $(\mathbf{a}, \mathbf{b})$, and then:
  ...
  $\mathbf{K}^T \mathbf{n}$ can be viewed as the backprojection of the 2D image line
  normal $\mathbf{n}$.

  $\mathbf{K}^T \mathbf{n}$ is indeed the normal of the backprojected plane
