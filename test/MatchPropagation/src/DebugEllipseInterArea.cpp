...
namespace DO {
  ...
  bool DebugEllipseInterArea::operator()(float inlierThres, float squaredEll)
  {
    ...
    for (int j = 1; j < 6; ++j)
    {
      ...
      drawer.displayImages();
      {
        ...
        for (size_t i = 0; i != inliers.size(); ++i)
        {
          ...
          float rel_diff_shape_mat = diff_shape_mat.squaredNorm()/y.shapeMat().squaredNorm();
          ...
          double error = (polyApproxOverlap - analyticalOverlap) / polyApproxOverlap;
          ...
          if (debug_ && (error > 0.2))
          {
            ...
            checkReprojectedEllipse(m, drawer, Sy, H_Sx, polyApproxOverlap, 
                                    analyticalOverlap, angle_phi_ox, angle_y, error);
    ...
    const string name( "inlierThres_" + toString(inlierThres)
  ...
  void DebugEllipseInterArea::checkReprojectedEllipse(const Match& m,
                                        ...
                                        double error) const
  {
    ...
    cout << "|Phi_theta_x - theta_y| = " << toDegree(abs(angle_phi_ox-angle_y)) << " deg" << endl;
    cout << endl;
