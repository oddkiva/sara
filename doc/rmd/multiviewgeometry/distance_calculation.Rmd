...
This section is a prelude to the next sections which presents more general
approaches.
...
loss of generality, let us choose a reference frame where the ice flow is
decribed by the plane $z = 0$. Note this would not really matter even if the ice
flow turns out to be inclined with respect to the sea level.
...
\begin{aligned}
  \cos \frac{\theta}{2} &= \frac{r}{\sqrt{r^2 + 4 f^2}} \\
  \sin \frac{\theta}{2} &= \frac{2f}{\sqrt{r^2 + 4 f^2}}
\end{aligned}
...
\begin{aligned}
  \cos \theta &= \frac{r^2 - 4f^2}{r^2 + 4 f^2} \\
...
A subtle yet important difference is that the light ray vector $\mathbf{x}$ for
...
Fortunately we can still relate the direction of the light ray and a ground
point $\mathbf{x}$ on the ice floe with the camera extrinsics $(\mathbf{R}',
\mathbf{t}')$.
...
By expanding the equations, we solve a linear system of 3x3 equations that
determines $\mathbf{x}$ and the distance $\lambda$ between the camera
and the penguin.
...
are calculated by solving the 3x3 linear system:

\begin{equation}
  \left\{ \begin{array}{lll}
  r'_{11} x + r'_{12} y - \lambda v_1 = -t'_{1} \\
  r'_{21} x + r'_{22} y - \lambda v_2 = -t'_{2} \\
  r'_{31} x + r'_{32} y - \lambda v_3 = -t'_{3} \\
  \end{array} \right.
\end{equation}
:::
:::
