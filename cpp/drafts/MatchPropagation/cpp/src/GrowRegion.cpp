...
namespace DO { namespace Sara {
  ...
  GrowRegion::operator()(const vector<Region>& RR, size_t maxRegionSize,
                         const PairWiseDrawer* pDrawer,
                         RegionGrowingAnalyzer* pAnalyzer)
  {
    Region R;
                                               ...
                                               RegionGrowingAnalyzer* pAnalyzer)
  {
    ...
    CHECK_STATE_AFTER_ADDING_SEED_MATCH(pDrawer)
  ...
  void GrowRegion::grow(const std::vector<Region>& RR, Region& R,
                        ...
                        size_t maxAllowedSize, const PairWiseDrawer* pDrawer,
                        RegionGrowingAnalyzer* pAnalyzer)
  {
    if (_verbose)
...
#pragma omp critical
                                 ...
                                 const PairWiseDrawer* pDrawer)
    {
      ...
      return find_triple(t, N_K_m_cap_R, pDrawer);
    }
...
#define I_KNOW_THE_ACCURACY_OF_LOCAL_AFFINITY
    ...
    void GrowRegion::check_local_affine_consistency(
        ...
        const Match m[4], const PairWiseDrawer* pDrawer) const
    {
      if (pDrawer)
      {
        // Check visually.
