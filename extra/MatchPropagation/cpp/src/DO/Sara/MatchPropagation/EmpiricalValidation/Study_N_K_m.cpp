...
namespace DO {
  ...
  bool Study_N_K_m::operator()(float inlierThres, float squaredEll, 
                               size_t K, double squaredRhoMin)
  {
    ...
    for (int j = 1; j < 6; ++j)
    {
      ...
      drawer.displayImages();
      {
        ...
        //ComputeMatchNeighborhood computeNeighborhood(filteredM, 1e3, &drawer, true);
        ...
        getStat(inlier_stat_N_K, inlier_stat_hatN_K, inlier_stat_diff, inliers, N_K, hatN_K);
        ...
        getStat(outlier_stat_N_K, outlier_stat_hatN_K, outlier_stat_diff, outliers, N_K, hatN_K);
    ...
    const string name( "squaredEll_" + toString(squaredEll)
                     + "_K_" + toString(K) 
                     + "_squaredRhoMin_" + toString(squaredRhoMin)
                     + dataset().featType()
                     + ".txt");
    // General stats
    ...
    cout << "Saved stats:\n" << string(folder+"/"+name) << endl;
    // Inlier stats
    if (!saveStats(folder_inlier+"/"+name, stat_N_Ks_inliers, stat_hatN_Ks_inliers, stat_diffs_inliers))
    {
    ...
    cout << "Saved stats:\n" << string(folder_inlier+"/"+name) << endl;
    // Outlier stats
    if (!saveStats(folder_outlier+"/"+name, stat_N_Ks_outliers, stat_hatN_Ks_outliers, stat_diffs_outliers))
    {
      cerr << "Could not save stats:\n" << string(folder+name) << endl;
      return false;
    }
  ...
  void
  Study_N_K_m::
  getStat(Stat& stat_N_K, Stat& stat_hatN_K, Stat& stat_diff,
  ...
  void
  Study_N_K_m::
  checkNeighborhood(const vector<vector<size_t> >& N_K,
  ...
  void
  Study_N_K_m::getStat(Stat& stat_N_K, Stat& stat_hatN_K, Stat& stat_diff,
  ...
  bool Study_N_K_m::saveStats(const string& name,
                              const vector<Stat>& stat_N_Ks,
