...
def test_aifi():
    ...
    with torch.no_grad():
        ...
        assert torch.dist(s5_pe, s5_pe_true, p=torch.inf) < 2e-6
        assert torch.dist(s5_pe, s5_pe_true) < 1e-4
        ...
        assert torch.dist(self_attn_out, self_attn_out_true) < 2e-4
        ...
        assert torch.dist(norm1_out, norm1_out_true) < 2e-4

        # Check the FFN.
        ffn_out = aifi_layer.feedforward(norm1_out)
        assert torch.dist(ffn_out, ffn_out_true) < 2e-4
    ...
    with torch.no_grad():
        assert torch.dist(norm2_out, aifi_out_true) < 2e-4
        ...
        dist = torch.dist(aifi_out, aifi_out_true)
