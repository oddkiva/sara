...
Now that we have introduced the notations, let us remind the equations used in
the Kalman filter.
...
obtain is from the temporal integration as in kinematics or classical Newtonian
mechanics and assuming that the state acceleration vector is constant:
...
\begin{align*}
  ...
  \dot{\mathbf{b}}_{k+1} &=
    \dot{\mathbf{b}}_k + \ddot{\mathbf{b}}_k (t_{k+1} - t_k) \\
  \ddot{\mathbf{b}}_{k+1} &= \ddot{\mathbf{b}}_{k}
\end{align*}
