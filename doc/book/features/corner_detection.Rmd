...
In other words, [@HarrisS:1988:alvey] suggest to not only compare every pixel
intensity in a small image patch to the central pixel intensity but to also
assign a cornerness score to the pixel at the center of the image patch by
summation.

This is a much better idea because we introduce a function that can be optimized
in a mathematical sense. Surely this cornerness function should respond
maximally at a corner location? And yes it is the case in practise.

Because $\mathbf{h}$ is small, we can approximate the summand in the cornerness
function with a first-order Taylor expansion
...
As pointed out by [@HarrisS:1988:alvey], we recognize the covariance matrix of local
