...
namespace DO::Sara {
  ...
  auto EpipolarEdgeAttributes::read_matches(
      H5File& h5_file, const ViewAttributes& view_attributes) -> void
  {
  ...
  auto EpipolarEdgeAttributes::resize_fundamental_edge_list() -> void
  {
    if (edges.empty())
      return;
    ...
    F_best_samples.resize(static_cast<int>(edges.size()),
                          FEstimator::num_points);
  }

  auto EpipolarEdgeAttributes::resize_essential_edge_list() -> void
  {
    if (edges.empty())
      return;
    ...
    E_best_samples.resize(static_cast<int>(edges.size()),
                          EEstimator::num_points);
  }
      ...
      const ViewAttributes& view_attributes, H5File& h5_file) -> void
  {
    ...
    std::for_each(
        std::begin(edge_ids), std::end(edge_ids), [&](const auto& ij) {
          ...
          h5_file.read_dataset(format("F_inliers/%d_%d", i, j), F_inliers[ij]);
        });
  }
      ...
      const ViewAttributes& view_attributes, H5File& h5_file) -> void
  {
    ...
    std::for_each(
        std::begin(edge_ids), std::end(edge_ids), [&](const auto& ij) {
          ...
          // Estimate the fundamental matrix.
          h5_file.read_dataset(format("E_inliers/%d_%d", i, j), E_inliers[ij]);
        });
  }
