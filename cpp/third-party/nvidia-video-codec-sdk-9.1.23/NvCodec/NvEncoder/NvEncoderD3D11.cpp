...
NvEncoderD3D11::NvEncoderD3D11(ID3D11Device* pD3D11Device, uint32_t nWidth,
                               uint32_t nHeight,
                               NV_ENC_BUFFER_FORMAT eBufferFormat,
                               uint32_t nExtraOutputDelay,
                               bool bMotionEstimationOnly,
                               bool bOutputInVideoMemory)
  : NvEncoder(NV_ENC_DEVICE_TYPE_DIRECTX, pD3D11Device, nWidth, nHeight,
...
void NvEncoderD3D11::AllocateInputBuffers(int32_t numInputBuffers)
{
  ...
  for (int count = 0; count < numCount; count++)
  {
    ...
    for (int i = 0; i < numInputBuffers; i++)
    {
      ...
      {
        NVENC_THROW_ERROR("Failed to create d3d11textures",
...
void NvEncoderD3D11::ReleaseD3D11Resources()
{
  ...
  for (uint32_t i = 0; i < m_vReferenceFrames.size(); ++i)
  {
    ...
    {
      reinterpret_cast<ID3D11Texture2D*>(m_vReferenceFrames[i].inputPtr)
          ->Release();
    }
  }
