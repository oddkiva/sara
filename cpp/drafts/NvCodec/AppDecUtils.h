...
//---------------------------------------------------------------------------
//! \file AppDecUtils.h
//! \brief Header file containing definitions of miscellaneous functions used by
//! Decode samples
//---------------------------------------------------------------------------
                            ...
                            bool* pbVerbose, int* piD3d)
{
  ...
  std::cout << "Options:" << std::endl
            ...
            << (piD3d ? "-d3d         9 (default): display with D3D9; 11: "
                        "display with D3D11\n"
                      : "");
  if (bThrowError)
  {
...
/**
 *   @brief  Function to generate space-separated list of supported video
 * surface formats
 *   @param  nOutputFormatMask - Bit mask to represent supported
  ...
  for (int iGpu = 0; iGpu < nGpu; ++iGpu)
  {
    ...
    for (auto i = 0u; i < sizeof(aeCodec) / sizeof(aeCodec[0]); ++i)
    {
      ...
      std::cout << "Codec  " << std::left << std::setw(7) << aszCodecName[i]
                ...
                << std::setw(7) << aszChromaFormat[decodeCaps.eChromaFormat]
                << "Supported  " << std::setw(3)
                << (int) decodeCaps.bIsSupported << "MaxWidth  " << std::setw(7)
                << decodeCaps.nMaxWidth << "MaxHeight  " << std::setw(7)
                << decodeCaps.nMaxHeight << "MaxMBCount  " << std::setw(10)
                << decodeCaps.nMaxMBCount << "MinWidth  " << std::setw(5)
                << decodeCaps.nMinWidth << "MinHeight  " << std::setw(5)
                << decodeCaps.nMinHeight << "SurfaceFormat  " << std::setw(11)
                << strOutputFormats << std::endl;
    }
