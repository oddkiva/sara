...
{
public:
  NvEncoderD3D11(ID3D11Device* pD3D11Device, uint32_t nWidth, uint32_t nHeight,
...
private:
   ...
   * NvEncoder::AllocateInputBuffers(). This function creates ID3D11Texture2D
   * textures which is used to accept input data. To obtain handle to input
   * buffers application must call NvEncoder::GetNextInputFrame()
   */
  virtual void AllocateInputBuffers(int32_t numInputBuffers) override;

private:
  /**
   *  @brief This is a private function to release ID3D11Texture2D textures used
   * for encoding.
   */
