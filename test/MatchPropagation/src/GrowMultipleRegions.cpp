...
namespace DO {
  ...
  GrowMultipleRegions::GrowMultipleRegions(const vector<Match>& M,
    ...
    , params_(params), verbose_(verbose)
  {
  }
  
  vector<Region>
  GrowMultipleRegions::operator()(size_t N,
                                  ...
                                  const PairWiseDrawer *pDrawer)
  {
    ...
    for (size_t m = 0; m != G_.size(); ++m)
    {
      ...
      if (!result.second.empty())
      {
        ...
        if (verbose_ >= 2)
        {
          cout << "[" << n << "] with match M[" << m << "]: Overlapping" << endl;
        ...
        mergeRegions(R_, result);
        // Remember not to grow from the following matches.
        markReliableMatches(allR, result.first);
      ...
      else if (result.first.size() > 7)
      {
        ...
        if (verbose_ >= 2)
        {
          cout << "[" << n << "] with match M[" << m << "]: Regular region growing" << endl;
          cout << "region size = " << result.first.size() << endl;
        }
    ...
    if (pAnalyzer)
    {
      pAnalyzer->setNumRegions( int(R_.size()) );
      pAnalyzer->setNumAttemptedGrowths( int(N) );
    }
    return R_;
  }

  void GrowMultipleRegions::markReliableMatches(Region& allR,
  ...
  void GrowMultipleRegions::mergeRegions(std::vector<Region>& Rs,
                                         const pair<Region, vector<size_t> >& result) const
  {
    // Merge regions.
    if (verbose_ >= 2)
    ...
    for (size_t i = 0; i != result.second.size(); ++i)
    {
      ...
      for (Region::iterator m = Rs[index].begin(); m != Rs[index].end(); ++m)
        mergedR.insert(*m);
      for (Region::iterator m = result.first.begin(); m != result.first.end(); ++m)
        mergedR.insert(*m);
    }
    if (verbose_ >= 2)
      cout << endl;

    // Erase the overlapping regions and put the region resulting from the merging
    vector<Region> newRs;
    newRs.reserve(1000);
  ...
  void
  GrowMultipleRegions::checkRegions(const vector<Region>& RR,
