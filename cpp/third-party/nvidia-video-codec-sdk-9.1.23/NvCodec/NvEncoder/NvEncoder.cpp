...
void NvEncoder::LoadNvEncApi()
{
  ...
  {
    NVENC_THROW_ERROR("Current Driver Version does not support this "
                      "NvEncodeAPI version, please upgrade driver",
...
void NvEncoder::GetInitializeParams(NV_ENC_INITIALIZE_PARAMS* pInitializeParams)
{
  ...
  {
    NVENC_THROW_ERROR("Both pInitializeParams and "
                      "pInitializeParams->encodeConfig can't be NULL",
                      NV_ENC_ERR_INVALID_PTR);
  }
...
NVENCSTATUS
NvEncoder::DoMotionEstimation(NV_ENC_INPUT_PTR inputBuffer,
