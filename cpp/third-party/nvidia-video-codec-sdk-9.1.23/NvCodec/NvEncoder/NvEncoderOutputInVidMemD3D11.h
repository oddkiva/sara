...
/**
 * @brief Class for encode or ME only output in video memory feature for D3D11
 * interfaces.
 */
...
public:
   ...
   */
  NvEncoderOutputInVidMemD3D11(ID3D11Device* pD3D11Device, uint32_t nWidth,
                               uint32_t nHeight,
   ...
   *  This function returns video memory buffer pointers containing compressed
   * data in pOutputBuffer. If there is buffering enabled, this may return
   * without any data in pOutputBuffer.
   */
   ...
   * destroying an encoder session. Video memory buffer pointer containing
   * compressed data is returned in pOutputBuffer.
   */
  ...
  /**
   *  @brief This function is used to get the size of output buffer required to
   * be allocated in order to store the output.
   */
  uint32_t GetOutputBufferSize();

private:
  /**
   *  @brief This function is used to allocate output buffers in video memory
   * for storing encode or motion estimation output.
   */
  ...
  /**
   *  @brief This function is used to unregister output resources which had been
   * previously registered for encoding using RegisterOutputResources()
   * function.
   */
  void UnregisterOutputResources();

  /**
   *  @brief This function is used to map the input and output buffers to
   * NvEncodeAPI.
   */
  void MapResources(uint32_t bfrIdx);

  /**
   *  @brief This is a private function which is used to get video memory buffer
   * pointer containing compressed data or motion estimation output from the
   * encoder HW. This is called by EncodeFrame() function. If there is buffering
   * enabled, this may return without any output data.
   */
  void GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR>& pOutputBuffer,
                        bool bOutputDelay);
