\newcommand{\eqdef}{\overset{\textrm{def}}{=}}
...
def calculate_normalizing_transform(center, shape, orientation_vector):
    # Calculate the angle.
    theta = np.arctan2(orientation_vector[1], orientation_vector[0])
...
```{python}
T = calculate_normalizing_transform(center, shape, orientation_vector)
print(T)
```
...
{
  // Calculate the angle.
  const auto theta = std::atan2(orientation_vector(1), orientation_vector(0));

  // Form the rotation matrix.
  const Eigen::Matrix2d Q = Eigen::Rotation2Dd{theta}.toRotationMatrix();

  // Decompose the ellipse matrix.
  const Eigen::Matrix2d M = (Q.transpose() * shape * Q).llt().matrixL();
