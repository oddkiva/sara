...
static const char*
GetVideoChromaFormatString(cudaVideoChromaFormat eChromaFormat)
{
  static struct
  {
  ...
  if (eChromaFormat >= 0 &&
      eChromaFormat < sizeof(aChromaFormatName) / sizeof(aChromaFormatName[0]))
  {
...
int NvDecoder::ReconfigureDecoder(CUVIDEOFORMAT* pVideoFormat)
{
  ...
  if (!bDecodeResChange && !m_bReconfigExtPPChange)
  {
    ...
    if (bDisplayRectChange)
    {
      ...
      m_nLumaHeight =
          pVideoFormat->display_area.bottom - pVideoFormat->display_area.top;
      m_nChromaHeight = int(m_nLumaHeight *
  ...
  if ((m_bReconfigExternal && bDecodeResChange) || m_bReconfigExtPPChange)
  {
    ...
    m_nChromaHeight =
        int(m_nLumaHeight * GetChromaHeightFactor(pVideoFormat->chroma_format));
    m_nNumChromaPlanes = GetChromaPlaneCount(pVideoFormat->chroma_format);
...
int NvDecoder::HandlePictureDisplay(CUVIDPARSERDISPINFO* pDispInfo)
{
      ...
      if (m_bUseDeviceFrame)
      {
        ...
        if (m_bDeviceFramePitched)
        {
          CUDA_DRVAPI_CALL(cuMemAllocPitch(
              (CUdeviceptr*) &pFrame, &m_nDeviceFramePitch, m_nWidth * m_nBPP,
  ...
  m.srcDevice =
      (CUdeviceptr)((uint8_t*) dpSrcFrame + m.srcPitch * m_nSurfaceHeight);
  m.dstDevice =
...
NvDecoder::NvDecoder(CUcontext cuContext, bool bUseDeviceFrame,
                     cudaVideoCodec eCodec, std::mutex* pMutex,
                     bool bLowLatency, bool bDeviceFramePitched,
                     const Rect* pCropRect, const Dim* pResizeDim, int maxWidth,
                     int maxHeight)
...
NvDecoder::~NvDecoder()
{
  ...
  {
    // LOG(WARNING) << "nFrameAlloc(" << m_nFrameAlloc << ") !=
    // m_vpFrame.size()(" << m_vpFrame.size() << ")";
  }
...
bool NvDecoder::Decode(const uint8_t* pData, int nSize, uint8_t*** pppFrame,
                       int* pnFrameReturned, uint32_t flags,
                       int64_t** ppTimestamp, int64_t timestamp,
                       CUstream stream)
{
  if (!m_hParser)
  {
  ...
  if (m_nDecodedFrame > 0)
  {
    if (pppFrame)
    {
      m_vpFrameRet.clear();
      std::lock_guard<std::mutex> lock(m_mtxVPFrame);
      m_vpFrameRet.insert(m_vpFrameRet.begin(), m_vpFrame.begin(),
...
bool NvDecoder::DecodeLockFrame(const uint8_t* pData, int nSize,
                                uint8_t*** pppFrame, int* pnFrameReturned,
                                uint32_t flags, int64_t** ppTimestamp,
                                int64_t timestamp, CUstream stream)
{
  bool ret = Decode(pData, nSize, pppFrame, pnFrameReturned, flags, ppTimestamp,
                    timestamp, stream);
  std::lock_guard<std::mutex> lock(m_mtxVPFrame);
