...
Consider two cameras $C_0$ and $C_1$. In the sequel, we can sort of
imagine that:

- the coordinate system of the first camera $C_0$ is the world coordinate
  system.
- the coordinate system of the second camera $C_1$ is a convenient local
  coordinate system.

Consider a 3D point with world coordinate system $\mathbf{X}^0$ and
suppose we can calculate its coordinates $\mathbf{X}^1$ in this local
...
w.r.t. camera $C_1$ (and in its local coordinate system), such that:
...
### Personal Intellectual Journey about Incremental Bundle Adjustment

In this paragraph I describe my personal intellectual journey to understand how
the incremental bundle adjustment is done.
