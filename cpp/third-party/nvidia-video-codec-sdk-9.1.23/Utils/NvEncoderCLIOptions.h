...
public:
                             ...
                             bool bOutputInVidMem = false)
  {
    ...
    oss << "-preset      Preset: "
        << (bLowLatency ? szLowLatencyPresetNames : szPresetNames) << std::endl
    ...
    oss << "-bitrate     Average bit rate, can be in unit of 1, K, M"
        << std::endl
    ...
    if (!bLowLatency)
    {
      oss << "-aq          Enable spatial AQ and set its stength (range 1-15, "
             "0-auto)"
          << std::endl
    ...
    if (!bUnbuffered && !bLowLatency)
    {
      oss << "-lookahead   Maximum depth of lookahead (range 0-32)"
          << std::endl;
    }
    oss << "-cq          Target constant quality level for VBR mode (range "
           "1-51, 0-auto)"
        << std::endl
  ...
  std::string MainParamToString(const NV_ENC_INITIALIZE_PARAMS* pParams)
  {
    ...
    os << std::endl
       ...
       << (pParams->encodeConfig->rcParams.enableLookahead
               ...
               : "disabled")
       << std::endl
       << "\tcq           : " << pParams->encodeConfig->rcParams.targetQuality
       << std::endl
       << "\tqmin         : P,B,I="
       << pParams->encodeConfig->rcParams.minQP.qpInterP << ","
       << pParams->encodeConfig->rcParams.minQP.qpInterB << ","
       << pParams->encodeConfig->rcParams.minQP.qpIntra << std::endl
       << "\tqmax         : P,B,I="
       << pParams->encodeConfig->rcParams.maxQP.qpInterP << ","
       << pParams->encodeConfig->rcParams.maxQP.qpInterB << ","
       << pParams->encodeConfig->rcParams.maxQP.qpIntra << std::endl
       << "\tinitqp       : P,B,I="
       << pParams->encodeConfig->rcParams.initialRCQP.qpInterP << ","
       << pParams->encodeConfig->rcParams.initialRCQP.qpInterB << ","
       << pParams->encodeConfig->rcParams.initialRCQP.qpIntra;
    return os.str();
  }

public:
  virtual GUID GetEncodeGUID()
  {
    return guidCodec;
  }
  virtual GUID GetPresetGUID()
  {
    return guidPreset;
  }
...
private:
  ...
  const char* szProfileNames =
      ...
      " stereo(h264) svc_temporal_scalability(h264) progressiv_high(h264) "
      "constrained_high(h264)"
...
public:
   ...
   */
  std::string
  FullParamToString(const NV_ENC_INITIALIZE_PARAMS* pInitializeParams)
  {
    std::ostringstream os;
    ...
    else if (pInitializeParams->encodeGUID == NV_ENC_CODEC_HEVC_GUID)
    {
      os << "NV_ENC_CODEC_CONFIG (HEVC):" << std::endl
         ...
         << "    pixelBitDepthMinus8: "
         << pConfig->encodeCodecConfig.hevcConfig.pixelBitDepthMinus8
         << std::endl
