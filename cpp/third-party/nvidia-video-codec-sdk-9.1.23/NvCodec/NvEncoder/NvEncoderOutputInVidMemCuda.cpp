...
NvEncoderOutputInVidMemCuda::NvEncoderOutputInVidMemCuda(
  ...
  : NvEncoderCuda(cuContext, nWidth, nHeight, eBufferFormat, 0,
...
void NvEncoderOutputInVidMemCuda::AllocateOutputBuffers(
  ...
  for (uint32_t i = 0; i < m_pOutputBuffers.size(); ++i)
  {
    if (m_pOutputBuffers[i])
    {
      NV_ENC_REGISTERED_PTR registeredPtr = RegisterResource(
          (void*) m_pOutputBuffers[i], NV_ENC_INPUT_RESOURCE_TYPE_CUDADEVICEPTR,
...
void NvEncoderOutputInVidMemCuda::CreateEncoder(
...
void NvEncoderOutputInVidMemCuda::EncodeFrame(
...
void NvEncoderOutputInVidMemCuda::EndEncode(
    std::vector<NV_ENC_OUTPUT_PTR>& pOutputBuffer)
{
  if (!IsHWEncoderInitialized())
  {
...
void NvEncoderOutputInVidMemCuda::GetEncodedPacket(
