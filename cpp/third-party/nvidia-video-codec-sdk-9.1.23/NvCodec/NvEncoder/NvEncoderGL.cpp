...
NvEncoderGL::NvEncoderGL(uint32_t nWidth, uint32_t nHeight,
                         NV_ENC_BUFFER_FORMAT eBufferFormat,
                         uint32_t nExtraOutputDelay, bool bMotionEstimationOnly)
  : NvEncoder(NV_ENC_DEVICE_TYPE_OPENGL, nullptr, nWidth, nHeight,
...
void NvEncoderGL::AllocateInputBuffers(int32_t numInputBuffers)
{
  ...
  for (int count = 0; count < numCount; count++)
  {
    ...
    {
      NV_ENC_INPUT_RESOURCE_OPENGL_TEX* pResource =
          new NV_ENC_INPUT_RESOURCE_OPENGL_TEX;
      uint32_t tex;
      ...
      glTexImage2D(GL_TEXTURE_RECTANGLE, 0, GL_R8,
                   ...
                   GetMaxEncodeHeight() + chromaHeight, 0, GL_RED,
    ...
    RegisterInputResources(
        inputFrames, NV_ENC_INPUT_RESOURCE_TYPE_OPENGL_TEX, GetMaxEncodeWidth(),
        GetMaxEncodeHeight(),
        GetWidthInBytes(GetPixelFormat(), GetMaxEncodeWidth()),
...
void NvEncoderGL::ReleaseGLResources()
{
  ...
  for (int i = 0; i < m_vInputFrames.size(); ++i)
  {
    ...
    {
      NV_ENC_INPUT_RESOURCE_OPENGL_TEX* pResource =
          (NV_ENC_INPUT_RESOURCE_OPENGL_TEX*) m_vInputFrames[i].inputPtr;
      if (pResource)
      {
  ...
  for (int i = 0; i < m_vReferenceFrames.size(); ++i)
  {
    ...
    {
      NV_ENC_INPUT_RESOURCE_OPENGL_TEX* pResource =
          (NV_ENC_INPUT_RESOURCE_OPENGL_TEX*) m_vReferenceFrames[i].inputPtr;
      if (pResource)
      {
        glDeleteTextures(1, &pResource->texture);
        delete pResource;
