...
$\mathbf{t}$ will naturally follow naturally from the first step.
...
Lambda-Twist [@PerssonN:2018:eccv] yourself in a mainstream computer languages
such as Python or C++. The publication [@PerssonN:2018:eccv] is a nice paper
but one criticism I can make is that it mixes $0$-based indices and $1$-based
...
:::fyi
Here I keep the $1$-based enumeration of the matrices $D_i$ because as I will
start proof-reading the paper [@PerssonN:2018:eccv] to convince you
:::
...
In general the quadric $\mathbf{D}_0$ is nonzero but it also has to be a
degenerate quadric (why?). This means that necessarily

:::fyi
I haven't understood why we want a degenerate quadric yet.
...
# Collect the coefficients c[i] of the polynomial as denoted in the paper.
c = det_D0.all_coeffs()
```

Now let us write our own formula of coefficients:

```{python, echo=TRUE, cache=TRUE}
D1 = sp.Matrix(D1)
D2 = sp.Matrix(D2)

# CAVEAT: SymPy enumerates coefficients in DECREASING degree order.
our_coeff = [
    # Our c[3]
    D2.det(),
...
for i in range(0, 4):
    print("c[{}] - our_coeff[{}] = {}".format(3 - i, 3 - i, sp.simplify(c[i] - our_coeff[i])))
```
...
[@Eberly:2014:eigen3x3]. I prefer follow [@Eberly:2014:eigen3x3]'s method since
the robustness outweighs the slight loss of speed.
...
manually recalculating the coefficients and double-checking with SymPy, instead
the coefficients.
...
# The physically feasible solution lambda vector.
lambda_valid = sp.Matrix([[w0 + tau * w1],
                          [1],
                          [tau]])

# The feasible solution must be in the second homogeneous quadric,
