...
Because of the wind, the drone does not necessarily have a perfect a zero pitch
angle. So this time let us imagine that the drone has to maintain its stationary
position with a nonzero pitch angle.
...
frame will be considered the reference frame.
...
3D light ray vector $-\mathbf{x}$. With the knowledge of the camera calibration
matrix $\mathbf{K}$, we can calculate $\mathbf{x}$ as
