...
about ellipse intersections. Specifically it details how to calculate the
intersection points and the intersection area. And it is not that a simple
geometric problem.

Originally I revisited :cite:`Eberly`'s study in the appendix of my thesis
:cite:`Ok:2013:phdthesis`. I am retouching my writing again to make it less dry
...
```{r ellsector2, echo = FALSE, fig.align = 'center', fig.cap = "The ellipse sector delimited by a line segment and the elliptical arc $(\\theta_1, \\theta_2)$ is colored in blue."}

knitr::include_graphics("figures/ellipticalsector2.png")
```

We are interested in calculating the elliptic portion delimited by a line
...
consider two ellipses $(\mathcal{E}_i)_{1 \leq i \leq 2}$ and we respectively
denote
...
-  the area of the elliptic portion bounded a line segment and an arc
   for ellipse $\mathcal{E}_i$ by $B_i$,

-  the number of intersection points by $L$,

-  the intersection points by $\mathbf{p}_i$ for
   $i \in \llbracket 1, L \rrbracket$, sorted in a counter-clockwise order,
   *i.e.*,
