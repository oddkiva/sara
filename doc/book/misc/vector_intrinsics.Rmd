...
this I mean billions (trillions?) of dollars 🤑💸💰🤑💸💰🤑💸💰] for any chip
...
$\mathbf{x}[t, :, :]$ and $\mathbf{y}[t, :, :]$ respectively, we simply denote
...
In pseudo-code, the **vectorized** FMA operation operates repeatedly on this 4D
...
is in a row-major storage format, all we do is just re-read the image tile data
in the cache, without ever needing to recopy the data in some appropriate order
in the memory: the source data is contiguous during the convolutional operation.
