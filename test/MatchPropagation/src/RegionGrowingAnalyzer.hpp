...
#ifndef DO_GROWREGION_REGIONGROWINGANALYZER_HPP
#define DO_GROWREGION_REGIONGROWINGANALYZER_HPP
...
namespace DO {
  ...
  public:
    RegionGrowingAnalyzer(const std::vector<Match>& M, const Matrix3f& H,
                          bool verbose = false)
      : M_(M), H_(H.cast<double>()), verbose_(verbose)
      , num_fusions_(0), num_regions_(0) {}
    ...
    void incrementNumFusions() { ++num_fusions_; }
    void setNumRegions(int numRegions) { num_regions_ = numRegions; }
    void setNumAttemptedGrowths(int numGrowths) { num_attempted_growths_ = numGrowths; }
    int numFusions() const { return num_fusions_; }
    int numRegions() const { return num_regions_; }
    int numGrowths() const { return num_attempted_growths_; }
    bool saveNumRegionStats(const std::string& name) const;

    //! Monitor the number of $\partial R$.
    void resetEvolDR() { size_dR_.clear(); good_.clear(); time_.clear(); }
    void push(size_t size_dR, size_t good, double time)
    { size_dR_.push_back(size_dR); good_.push_back(good); time_.push_back(time); }
  ...
  private:
     ...
     *  $\| \mathbf{H} \mathbf{x} - \mathbf{y} \|_2 \in [\delta_i, \delta_{i+1}[$.
     *  with 
...
#endif /* DO_GROWREGION_REGIONGROWINGANALYZER_HPP */