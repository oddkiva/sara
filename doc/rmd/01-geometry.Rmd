...
published in [@ok:hal-00743044] and which I also made available in **Sara**. I
did not want to do polygonal approximations and instead wanted to have a
closed-form approach if possible at all.

Then I found Eberly's paper [@Eberly] that provides a nice explanation
...
[@Ok:2013:phdthesis] and I have retouched the original text here a bit to make
it a bit more human (less dry) in this documentation.
...
Let us start with an ellipse $\mathcal{E}$ with semi-major axis $a$ and
semi-minor axis $b$, *i.e.*, $a \geq b > 0$. We suppose that $\mathcal{E}$ is
centered at the origin and is axis-aligned oriented, *i.e*., such that the axis
$a$ is along the $x$-axis and the axis $b$ along the $y$-axis. Then,
...
In case our math skill feels rusty, it is worth reminding ourselves how to
...
knitr::include_graphics(rep("../sphinx/source/figures/ellipsearea.png"))
```

Using the ellipse equation \@ref(eq:ellipse) and exploiting the symmetry of the
ellipse, the area of ellipse $\mathcal{E}$ is $4$ times the upper quadrant area
of the ellipse, *i.e.*,
...
```{r ellsector, echo = FALSE, fig.align = 'center', fig.cap = "The ellipse sector delimited by the polar angles $(\\theta_1, \\theta_2)$ is colored in blue"}

knitr::include_graphics(rep("../sphinx/source/figures/ellipticalsector.png"))
```
...
\begin{equation}
  A(\theta_1, \theta_2) =
    \frac{1}{2} \int_{\theta_1}^{\theta_2} r^2 \mathop{\mathrm{d}\theta}.
\end{equation}
...
Differentiating $t=\tan\theta$, $\mathop{\mathrm{d}t} =
\frac{\mathop{\mathrm{d}\theta}}{\cos^2(\theta)}$, it follows that
...
Thus we arrive to the following first result:
...
\end{equation}
```
However the integral is properly defined only in the interval $(\theta_1,
\theta_2) \in ]-\pi/2, \pi/2[$. Exploiting the ellipse symmetry, we could
retrieve the elliptical sector for any $(\theta_1, \theta_2) \in ]-\pi, \pi[$
...
knitr::include_graphics(rep("../sphinx/source/figures/ellipticalsector2.png"))
```

We are interested in computing the elliptic portion by a line segment and the
elliptical arc $(\theta_1, \theta_2)$ such that $|\theta_2 - \theta_1| \leq \pi$
...
where $(x_i,y_i) = (r_i\cos\theta_i, r_i\sin\theta_i)$ and $\displaystyle r_i =
\frac{ab}{\sqrt{b^2 \cos^2(\theta_i)+a^2 \sin^2(\theta_i)}}$ for $i \in
\{1,2\}$.

Note that the other portion corresponding to the red one in
Figure \@ref(fig:ellsector2) has an area which is equal to
$\pi a b - B(\theta_1, \theta_2) \geq B(\theta_1, \theta_2)$ if
$|\theta_2 - \theta_1| \leq \pi$.

To summarize, our portion of interest, highlighted by the blue elliptic
portion in Figure \@ref(fig:ellsector2), has an area which is equal to

```{proposition, name="Truncated Elliptic Sector Area"}
...
\mathcal{S}^{++}_2$ is a positive definite matrix defined
...
Note by construction Equation \@ref(eq:sigma-eps) is a singular value
...
presentation complements [@Eberly].
...
2}$. The two equations \@ref(eq:twoellipses) simultaneously holds, which leads
us to solve the following system of equation
...
point. Then we can calculate the $x$-coordinate of this intersection point.

To do so, we multiply the first equation by $q_2(y)$ and the second equation by
$p_2(y)$
...
Then subtracting the first equation from the second equation, the monomial $x^2$
disappears. Thus:
...
We observe that the vector $[1, x, x^2, x^3]^T$ is never zero for any real value
$x$. Thus necessarily the nullspace $\mathrm{Null}(\mathbf{B}(y))$ is always
...
Our presentation complements [@Eberly]. In the rest of the section, we
...
-  the axes of ellipse :math:`\mathcal{E}_i` by :math:`(a_i, b_i)`, the
   ...
   .. math::
      ...
      \begin{align}
        ...
        \mathbf{v}_i &\overset{\textrm{def}}{=}\begin{bmatrix} -\sin(\theta_i) \\ \cos(\theta_i) \end{bmatrix}\end{align}
...
Figure [:ref:`figinter2`], which [@Eberly] apparently forgot to consider.
