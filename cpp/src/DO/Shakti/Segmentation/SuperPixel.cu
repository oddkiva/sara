...
namespace DO { namespace Shakti {
  ...
  __device__ inline float squared_distance(const Vector2f& x1,
                                           const Vector4f& I1,
                                           const Vector2f& x2,
                                           const Vector4f& I2)
  {
    return (I1 - I2).squared_norm() +
           distance_weight * (x1 - x2).squared_norm();
  }
                               ...
                               const Vector4f* in_image)
  {
    ...
    // In each block, the 3x3 top-left threads will populate the list of the
    // cluster centers. The other threads will wait.
    if (x_t < 3 && y_t < 3)
    {
      x1 = blockIdx.x == 0 ? 1 : 0;
      y1 = blockIdx.y == 0 ? 1 : 0;
      ...
      if (0 <= x_b && x_b < num_clusters.x() && 0 <= y_b &&
          y_b < num_clusters.y())
      {
...
#pragma unroll
      for (int y = y1; y < y2; ++y)
      {
        ...
        if (d < nearest_distance)
        {
          nearest_distance = d;
          nearest_cluster_idx =
              Vector2i{blockIdx.x + x - 1, blockIdx.y + y - 1};
        }
      }
    }
  ...
  __global__ void update_means(Cluster* out_clusters, const int* in_labels,
                               const Vector4f* in_image)
  {
    // Be very careful with vicious issues due to memory padding!
    const auto cluster_pos = coords<2>();
    const auto cluster_idx =
        cluster_pos.x() + num_clusters.x() * cluster_pos.y();
...
namespace DO { namespace Shakti {
  ...
  SegmentationSLIC::SegmentationSLIC(const Vector2i& input_cluster_sizes)
    : _cluster_sizes{input_cluster_sizes}
  {
    SHAKTI_SAFE_CUDA_CALL(
        cudaMemcpyToSymbol(cluster_sizes, &_cluster_sizes, sizeof(Vector2i)));
  ...
  Vector2i SegmentationSLIC::get_image_sizes() const
  {
    auto sizes = Vector2i{};
    SHAKTI_SAFE_CUDA_CALL(
        cudaMemcpyFromSymbol(&sizes, image_sizes, sizeof(Vector2i)));
    return sizes;
  }
  ...
  int SegmentationSLIC::get_image_padded_width() const
  {
    ...
    SHAKTI_SAFE_CUDA_CALL(
        cudaMemcpyFromSymbol(&padded_width, image_padded_width, sizeof(int)));
    return padded_width;
  }
  ...
  void SegmentationSLIC::set_image_sizes(const Vector2i& sizes,
                                         int padded_width)
  {
    SHAKTI_SAFE_CUDA_CALL(
        cudaMemcpyToSymbol(image_sizes, &sizes, sizeof(Vector2i)));

    SHAKTI_SAFE_CUDA_CALL(
        cudaMemcpyToSymbol(image_padded_width, &padded_width, sizeof(int)));

    // Set the image grid for CUDA.
    _image_block_sizes = dim3(_cluster_sizes.x(), _cluster_sizes.y());
  ...
  void
  SegmentationSLIC::set_image_sizes(const MultiArray<Vector4f, 2>& device_image)
  {
  ...
  {
    SHAKTI_SAFE_CUDA_CALL(cudaMemcpyToSymbol(Shakti::distance_weight,
                                             &distance_weight, sizeof(int)));
  }

  MultiArray<int, 2> SegmentationSLIC::
  operator()(const MultiArray<Vector4f, 2>& image)
  {
  ...
  void SegmentationSLIC::operator()(int* labels, const Vector4f* rgba_image,
                                    const int* sizes)
  {
  ...
  SegmentationSLIC::init_clusters(const MultiArray<Vector4f, 2>& image)
  {
    ...
    SHAKTI_SAFE_CUDA_CALL(cudaMemcpyToSymbol(Shakti::num_clusters,
                                             &num_clusters, sizeof(Vector2i)));
  ...
  SegmentationSLIC::init_labels(const MultiArray<Vector4f, 2>& image)
  {
    ...
    _labels_grid_sizes = grid_size_2d(labels, _labels_block_sizes);

    return labels;
  }

  void SegmentationSLIC::assign_means(MultiArray<int, 2>& labels,
  ...
  void SegmentationSLIC::update_means(MultiArray<Cluster, 2>& clusters,
