...
namespace DO::Sara {
  ...
  bool EvalQualityOfLocalAffApprox::operator()(float squaredEll,
    ...
    for (int j = 1; j < 6; ++j)
    {
      ...
      // The job is here.
      {
        ...
        for (size_t lb = 0; lb != thres.size() - 1; ++lb)
        {
          int ub = lb + 1;
          bool success;
          success = doTheJob(M, sortedM, H, j, squaredEll, thres[lb], thres[ub],
...
#pragma omp critical
    ...
    const string name(
        ...
        "_K_" + toString(K) + "_rhoMin_" + toString(rho_min) + "_lb_" +
        toString(lb) + "_ub_" + toString(ub) + dataset().featType() + ".txt");

    bool success;
#pragma omp critical
    {
      success =
          analyzer.saveLocalAffineConsistencyStats(string(folder + "/" + name));
    }
    if (!success)
    {
