...
inline std::set<Eigen::Matrix<T, M, N, Opts, MaxRows, MaxCols>,
                DO::Sara::LexicographicalOrder>
to_std_set(const std::vector<Eigen::Matrix<T, M, N, Opts, MaxRows, MaxCols>>& v)
{
  return std::set<Eigen::Matrix<T, M, N, Opts, MaxRows, MaxCols>,
                  DO::Sara::LexicographicalOrder>(v.begin(), v.end());
}
