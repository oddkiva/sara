...
structure-from-motion, SIFT is a brilliant demonstration that traditional
approaches^[crafted through human intelligence :)] can still be outcompete
machine-learned features in some instances.
...
In more details, this is so that we can compensate for the *noisy* estimation of
keypoint $k$. If we did not apply the trilinear interpolation, a pixel at the
boundary of a subpatch patch could fall suddenly in another adjacent image
patch. As illustrated in Figure \@ref(fig:patches-nonoverlapping), this could
have a catastrophic effect in the performance of the SIFT descriptor.

Figure \@ref(fig:patches-nonoverlapping) tells a much longer story, which goes
as follows. Suppose that there is an identifiable keypoint with ground-truth
localization $k = (x = 0, y = 0, \theta = 0, \sigma = 1)$. Translating in R
language yields:
...
As animated in Figure \@ref(fig:patches-overlapping), by smoothly varying these
...
histogram of gradients computed in each subpatch will sharply diverge represent
from the ground truth histogram for small deviations.
...
To mitigate such boundary effect that could be catastrophic, [@Lowe:2004:ijcv]
resorts to the trilinear interpolation. That means that we have to increase the
radius of each subpatch (i, j) in the grid. In turn this causes them to overlap
each other as shown in Figure \@ref(fig:patches-radius-increase). Each subpatch
has their radius increase from $0.5$ to $1$.

As highlighted with the color blending in Figure
\@ref(fig:patches-radius-increase), the blue cross will this time belong to $4$
subpatches: the lavender, cyan, red and green subpatches.
...
We talked mostly about the benefit due the patch overlap but for the same
reason, [@Lowe:2004:ijcv] extends the idea to orientation bins. Orientations are
binned into $O$ principal orientations and again with the trilinear
interpolation, we compensate for the noisy orientation $\theta$. This leads us
...
In the end, image gradient contributes to at most $8$ SIFT bins $\mathbf{h}[i,
j, o]$.
:::
...
Equivalently with array broadcasting (as in *NumPy* concept):
