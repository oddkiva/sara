...
- a center with pixel coordinates $\x \in \mathbb{R}^2$,
  which is the only information that structure-from-motion pipelines use in the
  end,
...
will be content with illutrating such features with Figure
...
that the affine shape is well estimated across different change of viewpoints
and would bring a superior advantage in the feature matching.

In my experience however, I found that SIFT still performs very well without the
affine shape adaptation [@MikolajczykTSZMSKVG:2005:ijcv] and performs better
than affine-invariant features in structure-from-motion. Besides the
computational burden due to the affine shape adaptation is quite significant and
tricky to implement^[I am not sure I got it right in my implementation
attempts], which has led me to conclude they are not really worth it.
...
literature and should parameter tuning becomes necessary, it is very easy to do
so as I found them quite straightforward to interpret.
