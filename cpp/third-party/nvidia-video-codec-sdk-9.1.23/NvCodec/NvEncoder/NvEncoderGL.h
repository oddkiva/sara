...
public:
   ...
   */
  NvEncoderGL(uint32_t nWidth, uint32_t nHeight,
...
private:
   ...
   * NvEncoder::AllocateInputBuffers(). This function creates OpenGL textures
   * which are used to hold input data. To obtain handle to input buffers, the
   * application must call NvEncoder::GetNextInputFrame(). An OpenGL context
   * must be current to the thread/process when calling this method.
   */
   ...
   * NvEncoder::ReleaseInputBuffers(). An OpenGL context must be current to the
   * thread/process when calling this method.
   */
