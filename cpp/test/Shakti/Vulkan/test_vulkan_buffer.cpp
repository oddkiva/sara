...
BOOST_AUTO_TEST_CASE(test_uniform_buffer_with_factory)
{
  namespace svk = DO::Shakti::Vulkan;
  ...
  const auto ubo = svk::BufferFactory{device}  //
                       .make_uniform_buffer<ModelViewProjectionStack>(1);
  BOOST_CHECK(static_cast<VkBuffer>(ubo) != nullptr);
  SARA_CHECK(ubo.size());

  // Finally, allocate some device memory that fulfils these such memory
  // requirements.
  auto device_memory = svk::DeviceMemoryFactory{physical_device, device}
                           .allocate_for_uniform_buffer(ubo);
  BOOST_CHECK(static_cast<VkDeviceMemory>(device_memory) != nullptr);
  SARA_CHECK(device_memory.size());

  // Bind the UBO to this device memory.
  ubo.bind(device_memory, 0);
}
