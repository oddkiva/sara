                 ...
                 const TensorView_<double, 2>& colors)
{
  ...
  auto X_data = const_cast<double*>(reinterpret_cast<const double*>(X.data()));
  auto X_tensor =
  ...
  geom_ply_file.add_properties_to_element(
      ...
      reinterpret_cast<std::uint8_t*>(X_tensor.data()), tinyply::Type::INVALID,
      0);

  auto colors_rgb8 = image_view(colors).convert<Rgb8>();
  geom_ply_file.add_properties_to_element(
      "vertex", {"red", "green", "blue"}, tinyply::Type::UINT8, colors.size(0),
