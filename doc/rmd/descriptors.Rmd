...
patch. As illustrated \@ref(fig:patches-nonoverlapping), this could have a
catastrophic effect in the performance of the SIFT descriptor.

**TODO: illustrate the blue cross and the deviation from the true localisation
in scale-space and orientation. One point belongs to only one patch.**
...
for (ii in 1:30) {
  ...
  draw_patch(patch_center_shifted, theta[ii], scale[ii], 0.5,
             color = "orangered")
...
for (ii in 1:30) {
  ...
  # The noisy image patch.
  draw_patch(patch_center_shifted, -theta[ii], scale2[ii], 0.5,
             color = "orangered")
...
To compensate the boundary effect, the trilinear interpolation means that we
have to increase the radius of each sub-patch (i, j) that compose the grid, which
causes them to overlap each other as shown in Figure
\@ref(fig:patches-radius-increase).
...
With the overlapping of sub-patches in the grid, the histogram of gradients can
reflect the photometric information more robustly agains the noisy estimation of
the keypoint localisation because every pixel in $\mathcal{P}$ belongs up to $4$
adjacent patches $\mathcal{P}_{i'j'}$ for $i \leq i' \leq i + 1$ and $j \leq j'
\leq j + 1$. We illustrate this benefit in Figure \@ref(fig:patches-overlapping).
...
Then we can encode the photometric information of each patch $\mathcal{P}_{ij}$
into a histogram $\mathbf{h}_{ij} \in \mathbb{R}^O$ of *gradient orientations*,
where the orientations are binned into $O$ principal orientations. Again with
trilinear interpolation, we compensate for the noisy orientation $\theta$ and as
a result:
