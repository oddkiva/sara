...
I believe I made a mistake here in this section. I will rework this paragraph as
the correct decomposition should be an *RQ-decomposition* instead, and **NOT** a
*QR*-decomposition.

Indeed it makes more sense to transform the elliptic patch into a circular
patch. Only after that, should we look for the dominant gradient on the circular
patch, which will determine the appropriate rotation.

This will require some more verification work from my end. Apologies. 🙏🙏🙏
:::
...
\begin{equation}
  \left\{
    \z \in \mathbb{R}^{2} | \z^\top (\bL^{T})^{-1} \bL^{-1} \z = 1
  \right\}
\end{equation}
```
:::
...
\begin{equation}
  \begin{bmatrix} u & v \end{bmatrix}
  (\bL^{-1})^\top \bL^{-1} \begin{bmatrix} u \\ v \end{bmatrix} =
  %
  \begin{bmatrix} \cos(t) & \sin(t) \end{bmatrix}
...
#| fig.align: center
#| fig.cap: >
#|   Geometric interpretation of the QR factorization of linear transform matrix $\bL_x$.
knitr::include_graphics(paste0(getwd(), "/figures/normalizingTransform.png"))
```
...
\begin{equation}
  \begin{aligned}
    \bSigma_x &= (\bL_x^{-1})^\top \bL_x^{-1} \\
              &= \bQ (\bR^{-1})^\top \bR^{-1} \bQ^\top \quad \text{since}\ \bQ^\top = \bQ^{-1}\\
    %
    \bQ^\top \bSigma_x \bQ &=  (\bR^{-1})^\top \bR^{-1}
  \end{aligned}
\end{equation}

We recognize the Cholesky decomposition of matrix $\bQ^\top \bSigma_x \bQ$
...
\begin{equation}
  \displaystyle \bT_x = \begin{bmatrix} \bL_x & \x \\ \mathbf{0}_2^\top & 1 \end{bmatrix}.
\end{equation}
...
- Decompose the ellipse matrix
  $\bM := \mathrm{Cholesky}(\bQ^\top \bSigma_x \bQ)$

- $\bM$ is a lower triangular matrix such that

  - $\bM \bM^\top = \bQ^\top \bSigma_x \bQ$
  - $\bR := (\bM^\top)^{-1}$
  - $\bL := \bQ \bR$
  - $\bT_x := \begin{bmatrix} \bL & \x \\ \mathbf{0}_2^\top & 1 \end{bmatrix}$
:::
